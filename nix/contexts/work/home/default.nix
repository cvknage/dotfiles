{
  ...
  programs.bash = {
    ...
    initExtra = ''
      ...
      export_sops_secret GITHUB_ORG "${config.sops.secrets.github_org.path}"
      if [ -n "''${GITHUB_TOKEN-}" ] && [ -n "''${GITHUB_ORG-}" ]; then
        export NIX_CONFIG="access-tokens = github.com/$GITHUB_ORG=$GITHUB_TOKEN"
      fi
