...
in {
  ...
  # profile denies "$(getconf DARWIN_USER_TEMP_DIR)/secrets.d" and the linux side denies
  # "secrets.d" under /run/user/<uid> (policy/paths.nix: runtimeCredentialDirs). Each encodes the
  # "%r" substitution, so any other value moves the secrets out from under every deny --
  ...
  assertions = lib.optional (lib.hasAttrByPath ["sops" "defaultSecretsMountPoint"] options) {
    ...
    message = ''
      ...
      nix/modules/home-manager/agents/sandbox/darwin.nix (and runtimeCredentialDirs in
      nix/lib/agents/policy/paths.nix on linux) to match.
    '';
  };
