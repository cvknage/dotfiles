---
name: human-review
description: Run the user's manual review pass over staged changes in Neovim/Fugitive after the automatic `agent-review` skill and before proposing a commit. Looks for REVIEW comments the user leaves in the staged files once they say they're done, then reasons about each one.
---
...
- **Hand off**: tell the user the files are staged and ready, and to start the review in Neovim — the review tooling opens a dedicated tab with the first staged file in a side-by-side diff split against `HEAD`, reused across the whole pass rather than opening one per file; toggling it again closes that tab. Stepping moves to the next/previous staged file (looping) within that same tab. The right-hand pane is the real, editable file on disk. Adding a review comment inserts a `REVIEW: ` line (using the buffer's `commentstring`, so it works in any filetype) below the cursor and drops into insert mode to type the note. Then wait — do not scan for comments until the user says they're finished.
